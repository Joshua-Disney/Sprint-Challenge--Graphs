...
# roomGraph={0: [(3, 5), {'n': 1, 's': 5, 'e': 3, 'w': 7}], 1: [(3, 6), {'s': 0, 'n': 2, 'e': 12, 'w': 15}], 2: [(3, 7), {'s': 1}], 3: [(4, 5), {'w': 0, 'e': 4}], 4: [(5, 5), {'w': 3}], 5: [(3, 4), {'n': 0, 's': 6}], 6: [(3, 3), {'n': 5, 'w': 11}], 7: [(2, 5), {'w': 8, 'e': 0}], 8: [(1, 5), {'e': 7}], 9: [(1, 4), {'n': 8, 's': 10}], 10: [(1, 3), {'n': 9, 'e': 11}], 11: [(2, 3), {'w': 10, 'e': 6}], 12: [(4, 6), {'w': 1, 'e': 13}], 13: [(5, 6), {'w': 12, 'n': 14, 'e': 18}], 14: [(5, 7), {'s': 13, 'e': 20}], 15: [(2, 6), {'e': 1, 'w': 16}], 16: [(1, 6), {'n': 17, 'e': 15}], 17: [(1, 7), {'s': 16}], 18: [(6, 6), {'n': 20, 'w': 13, 'e': 19}], 19: [(7, 6), {'w': 18}], 20: [(6, 7), {'w': 14, 's': 18}]}
...
def maze_runner():
    ...
    while stack.size() > 0:
        ...
        print(", ".join(str(i.name) for i in current_path))
        ...
        if "n" in current_room.getExits() and room_descriptions[current_room.name]["n"] is "?":
            ...
            stack.push(new_path)
        elif "?" not in room_descriptions[current_room.name].values():
            ...
            while queue.size() > 0:
                ...
                if vertex.name not in checked:
                    ...
                    if "?" in room_descriptions[vertex.name].values():
                    ...
                    elif "e" in vertex.getExits() and vertex.e_to.name not in checked:
                        ...
                        bfs_path.append("e")
                        queue.enqueue(test_path)
                    elif "w" in vertex.getExits() and vertex.w_to.name not in checked:
                        ...
                        bfs_path.append("w")
                        queue.enqueue(test_path)
                    elif "n" in vertex.getExits() and vertex.n_to.name not in checked:
