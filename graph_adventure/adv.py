...
def findAllPaths():
    ...
    print(f"current room: {player.currentRoom.name}.  Coordinates: {player.currentRoom.description}")
    ...
    print(f"room_descriptions 2: {room_descriptions}")
    print("Possible directions:")
    for i in range(0, len(player.currentRoom.getExits())):
        print(f"{player.currentRoom.getExits()[i]} to: {player.currentRoom.n_to.name}")
    print(f"{len(visited_rooms)} visited room(s).")
    while len(visited_rooms) < len(roomGraph):
        if "n" in player.currentRoom.getExits():
            if room_descriptions[player.currentRoom.name]["n"] is "?":
                print("North is unexplored.")
                traversalPath.append("n")
                print(f"traversal path: {traversalPath}")
                room_descriptions[player.currentRoom.name]["n"] = player.currentRoom.n_to.name
                print(f"room descriptions 3: {room_descriptions}")
                last_room = player.currentRoom.name
                ...
                break
        elif "e" in player.currentRoom.getExits():
            print("Can't go North from here but I can go east.")
            break
        elif "s" in player.currentRoom.getExits():
            print("Already gone north and can't go east from here but I can go south.")
            if room_descriptions[player.currentRoom.name]["s"] is "?":
                print("South is unexplored.")
                traversalPath.append("s")
                print(f"traversal path: {traversalPath}")
                room_descriptions[player.currentRoom.name]["s"] = player.currentRoom.s_to.name
                print(f"room descriptions 3: {room_descriptions}")
                last_room = player.currentRoom.name
                player.travel("s", False)
                visited_rooms.add(player.currentRoom)
                print(f"Travelled south.  New room is {player.currentRoom.name}")
                print(f"Rooms visited: {len(visited_rooms)}")
                room_descriptions[player.currentRoom.name] = {}
                for i in range(0, len(player.currentRoom.getExits())):
                    room_descriptions[player.currentRoom.name][player.currentRoom.getExits()[i]] = "?"
                ...
                player.travel("s", False)
                print(f"Travelled south.  New room is {player.currentRoom.name}")
        elif "w" in player.currentRoom.getExits():
