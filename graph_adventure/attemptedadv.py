...
world.printRooms()
player = Player("Disney", world.startingRoom)
...
#     print(f"{len(visited_rooms)} visited room(s).")
...
def findAllPaths():
    ...
    visited_rooms = set()
    player.currentRoom = world.startingRoom
    room_descriptions[player.currentRoom.name] = {}
    print("Adding directions.")
    for i in range(0, len(player.currentRoom.getExits())):
        room_descriptions[player.currentRoom.name][player.currentRoom.getExits()[i]] = "?"
    print(f"room_descriptions 1: {room_descriptions}")
    queue = Queue()
    ...
    while queue.size() > 0:
        ...
        if current_room not in visited_rooms:
            ...
            print(f"current room: {current_room.name}.  Coordinates: {current_room.description}")
            # print("Possible directions:")
            # if "n" in current_room.getExits():
            #     print(f"n to {current_room.n_to.name}")
            # if "s" in current_room.getExits():
            #     print(f"s to {current_room.s_to.name}")
            # if "w" in current_room.getExits():
            #     print(f"w to {current_room.w_to.name}")
            # if "e" in current_room.getExits():
                # print(f"e to {current_room.e_to.name}")
            print(f"{len(visited_rooms)} visited room(s).")
            if "n" in current_room.getExits():
                print("North exists")
                if room_descriptions[current_room.name]["n"] is "?":
                    going_north = True
                    print("North is unexplored.")
                    room_descriptions[current_room.name]["n"] = current_room.n_to.name
                    print(f"room descriptions 2: {room_descriptions}")
                    last_room = current_room.name
                    player.travel("n", False)
                    traversalPath.append("n")
                    print(f"traversal path: {traversalPath}")
                    # visited_rooms.add(current_room)
                    # print(f"Travelled north.  New room is {player.currentRoom.name}")
                    # print(f"Rooms visited: {len(visited_rooms)}")
                    # # room_descriptions[current_room.name] = {}
                    room_descriptions[player.currentRoom.name] = {}
                    for i in range(0, len(player.currentRoom.getExits())):
                        room_descriptions[player.currentRoom.name][player.currentRoom.getExits()[i]] = "?"
                        room_descriptions[player.currentRoom.name]["s"] = last_room
                    print(f"room descriptions 3: {room_descriptions}")
                    queue.enqueue(player.currentRoom)
                else:
                    print("North has been explored but we need to go back.")
                    break
            elif "e" in current_room.getExits():
                print("Can't go North from here but I can go east.")
                if room_descriptions[current_room.name]["e"] is "?":
                    going_east = True
                    print("East is unexplored.")
                    room_descriptions[current_room.name]["e"] = current_room.s_to.name
                    print(f"room descriptions 2: {room_descriptions}")
                    last_room = current_room.name
                    ...
                    for i in range(0, len(player.currentRoom.getExits())):
                        room_descriptions[player.currentRoom.name][player.currentRoom.getExits()[i]] = "?"
                        room_descriptions[player.currentRoom.name]["w"] = last_room
                    print(f"room descriptions 3: {room_descriptions}")
                    queue.enqueue(player.currentRoom)
                else:
                    print("East has been explored but we need to go back.")
                    break
            elif "s" in current_room.getExits():
                print("Already gone north and can't go east from here but I can go south.")
                if room_descriptions[current_room.name]["s"] is "?":
                    print("South is unexplored.")
                    room_descriptions[current_room.name]["s"] = current_room.s_to.name
                    print(f"room descriptions 2: {room_descriptions}")
                    last_room = current_room.name
                    ...
                    for i in range(0, len(player.currentRoom.getExits())):
                        room_descriptions[player.currentRoom.name][player.currentRoom.getExits()[i]] = "?"
                        room_descriptions[player.currentRoom.name]["n"] = last_room
                    print(f"room descriptions 3: {room_descriptions}")
                    queue.enqueue(player.currentRoom)
                # if room_descriptions[current_room.name]["s"] is "?":
                #     print("South is unexplored.")
                ...
                #     player.travel("s", False)
                #     visited_rooms.add(current_room)
                #     print(f"Travelled south.  New room is {current_room.name}")
                #     print(f"Rooms visited: {len(visited_rooms)}")
                #     room_descriptions[current_room.name] = {}
                #     for i in range(0, len(current_room.getExits())):
                #         room_descriptions[current_room.name][current_room.getExits()[i]] = "?"
                #         room_descriptions[current_room.name]["n"] = last_room
                #     print(f"room descriptions 4: {room_descriptions}")
                else:
                    print("South has been explored but we need to go back.")
                    traversalPath.append("s")
                    print(f"traversal path: {traversalPath}")
                    last_room = current_room.name
                    player.travel("s", False)
            ...
            #         print(f"Travelled south.  New room is {current_room.name}")
                ...
                print("Already gone north and east and can't go South from here but I can go west.")
                if room_descriptions[current_room.name]["w"] is "?":
                    print("West is unexplored.")
                    room_descriptions[current_room.name]["w"] = current_room.s_to.name
                    print(f"room descriptions 2: {room_descriptions}")
                    last_room = current_room.name
                    ...
                    for i in range(0, len(player.currentRoom.getExits())):
                        room_descriptions[player.currentRoom.name][player.currentRoom.getExits()[i]] = "?"
                        room_descriptions[player.currentRoom.name]["e"] = last_room
                    print(f"room descriptions 3: {room_descriptions}")
                    queue.enqueue(player.currentRoom)
                else:
                    print("West has been explored but we need to go back.")
                    break
        else:
            print(f"I've already been to this room before.")
            if "n" in current_room.getExits():
                if room_descriptions[current_room.name]["n"] is not "?":
                    print(f"I've already gone north.")
                    going_north = False
            if "e" in current_room.getExits():
                if room_descriptions[current_room.name]["e"] is not "?":
                    print(f"I've already gone east.")
                    going_east = False
                elif going_north is False:
                    # print("South has been explored but we need to go back.")
                    traversalPath.append("e")
                    print(f"traversal path: {traversalPath}")
                    last_room = current_room.name
                    ...
                    queue.enqueue(player.currentRoom)
            if "s" in current_room.getExits():
                if room_descriptions[current_room.name]["s"] is not "?":
                    print(f"I've already gone south.")
                if going_north is False and going_east is False:
                    # print("South has been explored but we need to go back.")
                    traversalPath.append("s")
                    print(f"traversal path: {traversalPath}")
                    last_room = current_room.name
                    ...
                    queue.enqueue(player.currentRoom)
            if "w" in current_room.getExits():
                if room_descriptions[current_room.name]["w"] is not "?":
                    print(f"I've already gone west.")
            
    if len(visited_rooms) == len(roomGraph):
        print(f"TESTS PASSED: {len(traversalPath)} moves, {len(visited_rooms)} rooms visited")
    print(f"final room descriptions: {room_descriptions}")
    print(f"final traversal path: {traversalPath}")
